...
export function Modal({ open, onClose, title, children, wide = false }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto p-2 sm:items-center sm:p-4" dir="rtl">
      <div className="absolute inset-0 bg-[#2c1b3d]/70 backdrop-blur-sm" onClick={onClose} aria-hidden="true" />
      <div
        role="dialog"
        aria-modal="true"
        className={`relative z-10 my-auto flex w-full max-h-[calc(100dvh-1rem)] min-h-0 flex-col overflow-hidden rounded-2xl bg-white shadow-2xl sm:max-h-[calc(100dvh-2rem)] ${wide ? "sm:max-w-4xl" : "sm:max-w-lg"}`}
      >
        <div className="flex shrink-0 items-center justify-between border-b border-gray-100 bg-white px-4 py-3 sm:px-6 sm:py-4">
          ...
          <button type="button" onClick={onClose} aria-label="إغلاق النافذة" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-gray-400 transition hover:bg-red-50 hover:text-red-500">
...
export const Input = ({ icon, className = "", ...props }) => {
  ...
  const controlClass = `w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm transition focus:border-[#eab308] focus:outline-none focus:ring-2 focus:ring-[#eab308]/30 min-h-[44px] ${isDateTime ? "date-time-input" : ""} ${className}`;
  const controlProps = isDateTime && !props.dir ? { ...props, dir: "ltr", lang: "en-GB" } : props;

  if (icon) {
    return (
      <div className="relative">
        <input {...controlProps} className={`${controlClass} pr-10`} />
        <i className={`fa-solid ${icon} absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-400`} />
      </div>
    );
  }

  return <input {...controlProps} className={controlClass} />;
};
...
export const Toggle = ({ on = false, onChange }) => (
  <button
    ...
    onClick={() => onChange?.(!on)}
    className={`relative inline-flex h-6 w-11 shrink-0 items-center rounded-full transition ${on ? "bg-emerald-500" : "bg-gray-300"}`}
  >
    <span className={`inline-block h-5 w-5 rounded-full bg-white shadow transition-transform ${on ? "translate-x-0.5" : "-translate-x-5"}`} />
  </button>
);
